...
def placeholder_from_space(space):
    ...
    elif isinstance(space, gymnasium.spaces.box.Box):
        print(space.shape, type(space.shape))
        #return placeholder(space.shape)
        return placeholder((space.shape[1], ))
    elif isinstance(space, Discrete) or isinstance(space, gymnasium.spaces.discrete.Discrete):
...
def mlp_actor_critic(x, a, hidden_sizes=(64,64), activation=tf.tanh,
    ...
    if policy is None and isinstance(action_space, Box) or isinstance(action_space, gymnasium.spaces.box.Box):
        policy = mlp_gaussian_policy
    elif policy is None and isinstance(action_space, Discrete) or isinstance(action_space, gymnasium.spaces.discrete.Discrete):
